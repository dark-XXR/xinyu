...
11. **邀请推广 (`/referrals`)**
    - **活动草稿与多版本跟踪**：支持创建新活动草稿与编辑已有活动，保存草稿自动推进草稿版本号；
    - **多阶里程碑奖励规则**：灵活配置不同阶段里程碑（账号认证/首充/累积消费）、受益人（邀请人/被邀请人）以及奖励单位（算力能量/文本额度/VIP天数）与冷却期；
    - **严格防作弊拦截**：配置自邀请拦截、同设备多账号拦截、同支付身份拦截、主渠道验证要求及风控复核分阈值（`riskReviewScore` 0..100）；
    - **灰度滚动发布**：发布活动支持设置 1%..100% 灰度比例、生效与失效时间以及至少 8 字审计理由；
    - **真实已发布历史版本回退**：版本回退对话框只加载并列出真实已发布的历史版本（`wasPublished === true` 且 `version < 当前版本`），使用 Select 控件安全选择，拒绝自由猜写版本号；
    - **全部数值后台配置**：页面业务组件不硬编码任何奖励数量、次数或风控阈值，动态 KPI 与摘要全部由后台 Query 返回数据实时计算。
